        ...
        <div className="mt-8 bg-[#12141a] rounded-xl p-6 border border-violet-500/10">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-6">
                ...
                <div className="col-span-2 lg:col-span-1 bg-gradient-to-r from-violet-500/10 to-blue-500/10 rounded-xl p-4 flex flex-col items-center justify-center">
                    ...
                    <div className={`mt-2 px-3 py-1 rounded-full text-xs ${risks.totalRisk <= 2 ? 'bg-green-500/20 text-green-400' :
                            risks.totalRisk <= 3 ? 'bg-yellow-500/20 text-yellow-400' :
                                'bg-red-500/20 text-red-400'
                        }`}>
                        {risks.totalRisk <= 2 ? 'Low Risk' :
                            risks.totalRisk <= 3 ? 'Medium Risk' :
                                'High Risk'}
                    </div>
                </div>
            </div>
            ...
            <div className="mt-6 p-4 bg-violet-500/5 rounded-xl">
                <div className="flex items-start space-x-3">
                    ...
                    <div className="text-sm text-gray-400">
                        Risk scores range from 1 (lowest risk) to 5 (highest risk). The total risk score is calculated based on chain security, protocol reliability, pool performance, and underlying asset stability.
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="bg-[#1a1d25] rounded-xl p-6">
            ...
            <div className="grid grid-cols-3 gap-8">
                            ...
                            >
                                <div className="flex flex-col space-y-2">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className={`px-2 py-1 rounded-full text-xs ${protocol.risk === 1 ? 'bg-green-500/20 text-green-400' :
                                                protocol.risk === 2 ? 'bg-yellow-500/20 text-yellow-400' :
                                                    'bg-red-500/20 text-red-400'
                                            }`}>
                                            Risk: {protocol.risk}
                                        </span>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex flex-col space-y-3">
                        {selectedChain && selectedProtocol && pairs[selectedChain]?.[selectedProtocol]?.map((pair) => (
                            <button
                                key={pair.id}
                            ...
                            >
                                <div className="flex flex-col space-y-2">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className={`px-2 py-1 rounded-full text-xs ${pair.risk === 1 ? 'bg-green-500/20 text-green-400' :
                                                pair.risk === 2 ? 'bg-yellow-500/20 text-yellow-400' :
                                                    'bg-red-500/20 text-red-400'
                                            }`}>
                                            Risk: {pair.risk}
                                        </span>
                    ...
                    <div className="mt-8 p-6 bg-[#12141a] rounded-xl border border-blue-500/10">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            ...
                            <div className="p-4 bg-cyan-500/10 rounded-lg">
                                ...
                                <div className="flex items-center justify-between">
                                    <span className="font-medium">{protocols[selectedChain].find(p => p.id === selectedProtocol)?.name}</span>
